...
class Categoria(models.Model):
    id = models.UUIDField(max_length=150, unique=True,primary_key=True, db_index=True)
...
class Produto(models.Model):
    id = models.UUIDField(max_length=100, unique=True, primary_key=True, db_index=True)
    nome = models.CharField(max_length=100, db_index=True)
    descricao = models.TextField(blank=True)
    preco = models.DecimalField(max_digits=10, decimal_places=2)
    categoria_id = models.ForeignKey(Categoria, related_name='categoria_id', null=True, on_delete=models.CASCADE)
    disponivel = models.BooleanField(default=True)
    estoque = models.PositiveIntegerField()
